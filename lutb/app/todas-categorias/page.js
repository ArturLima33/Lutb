...
export default function TodasCategorias() {
  ...
  const categoriasOrdenadas = [...categorias].sort((a, b) => {
    ...
    if (ordenacao === "padrao" || ordenacao === "recentes") return dataB - dataA;
    ...
    <div style={{ backgroundColor: "#76BA5B", minHeight: "100vh", padding: "40px 20px", display: "flex", flexDirection: "column", alignItems: "center" }}>
      ...
      {categorias.length > 0 && (
        <div style={{ display: "flex", justifyContent: "center", marginBottom: "25px" }}>
          <select value={ordenacao} onChange={(e) => setOrdenacao(e.target.value)} style={{ backgroundColor: "#2D2D2D", color: "white", border: "none", borderRadius: "14px", padding: "12px 24px", fontSize: "16px", cursor: "pointer", outline: "none" }}>
        ...
        <div style={{ width: "100%", display: "flex", flexDirection: "column", gap: "20px" }}>
          {categoriasOrdenadas.map((c) => (
            <Link href={`/categoria/${c.id}`} key={c.id} style={{ textDecoration: "none" }}>
              <div style={{ backgroundColor: "white", borderRadius: "25px", padding: "20px 35px", boxShadow: "0 4px 10px rgba(0,0,0,0.1)", display: "flex", alignItems: "center", gap: "20px", minHeight: "85px" }}>
                {c.imagem && <img src={c.imagem} alt={c.nome} style={{ width: "55px", height: "55px", objectFit: "cover", borderRadius: "50%" }} />}
                <h3 style={{ fontSize: "22px", margin: 0, fontFamily: "serif", color: "#2D2D1A", fontWeight: "normal" }}>{c.nome}</h3>
              </div>
            </Link>
          ))}
        </div>
