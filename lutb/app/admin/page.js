...
export default function Admin() {
  ...
  const [sobreMural, setSobreMural] = useState([]);
  const [novaMuralUrl, setNovaMuralUrl] = useState("");
  const [arquivoMural, setArquivoMural] = useState("");
  ...
  useEffect(() => {
    ...
    setSobreTexto(textoSalvo);
    setSobreMural(muralSalvo);
  }, []);
  ...
  const removerArquivoAdicional = (index) => setArquivosLocaisAdicionais(arquivosLocaisAdicionais.filter((_, i) => i !== index));

  const moverArquivo = (origem, destino) => {
    const novaLista = [...arquivosLocaisAdicionais];
    const item = novaLista[origem];
  ...
  const salvarProduto = async () => {
    ...
    if (produtoId && todasImagensAdicionais.length > 0) {
      await supabase.from("produto_imagem").insert(
        todasImagensAdicionais.map((url, i) => ({ produto_id: produtoId, url, ordem: i }))
      );
    }
  ...
  const editarProduto = async (p) => {
    ...
    if (p.img && p.img.includes("supabase.co/storage")) {
      setArquivoLocalImg(p.img); setImg("");
    } else {
      setImg(p.img || ""); setArquivoLocalImg("");
    }

    const { data: imgs } = await supabase.from("produto_imagem").select("url").eq("produto_id", p.id).order("ordem");
    const urls = (imgs || []).map(i => i.url);
    setImagens(urls.filter(u => !u.includes("supabase.co/storage")));
    setArquivosLocaisAdicionais(urls.filter(u => u.includes("supabase.co/storage")));
  ...
  const limparFormCategoria = () => {
    setNomeCategoria(""); setImagemCategoria(""); setArquivoLocalCategoria(""); setEditandoCategoria(null);
  };
  ...
  const editarCategoria = (c) => {
    setNomeCategoria(c.nome); setEditandoCategoria(c.id);
    if (c.imagem && c.imagem.includes("supabase.co/storage")) {
      setArquivoLocalCategoria(c.imagem); setImagemCategoria("");
    } else {
      setImagemCategoria(c.imagem || ""); setArquivoLocalCategoria("");
    }
  };
  ...
  const gerenciarCheckboxHome = (marcado) => {
    if (marcado) {
      ...
      if (jaFixados >= 2) { alert("Limite máximo atingido! Só é possível fixar até 2 coleções na home."); return; }
    }
    setFixarHome(marcado);
  };

  const limparFormColecao = () => {
    setNomeColecao(""); setIconeColecao(""); setArquivoLocalColecao(""); setCorColecao("#987317"); setFixarHome(false); setEditandoColecao(null);
  };
  ...
  const editarColecao = (c) => {
    ...
    if (c.icone && c.icone.includes("supabase.co/storage")) {
      setArquivoLocalColecao(c.icone); setIconeColecao("");
    } else {
      setIconeColecao(c.icone || ""); setArquivoLocalColecao("");
    }
  };
  ...
  const salvarSobreTexto = () => {
    localStorage.setItem("sobre-texto", sobreTexto);
    alert("Texto salvo!");
  };

  const adicionarFotoMural = async () => {
    const url = arquivoMural || novaMuralUrl.trim();
    if (!url) { alert("Adicione uma URL ou faça upload de uma imagem."); return; }
    const novoMural = [...sobreMural, url];
    setSobreMural(novoMural);
    localStorage.setItem("sobre-mural", JSON.stringify(novoMural));
    setNovaMuralUrl(""); setArquivoMural("");
  };

  const removerFotoMural = (index) => {
    const novoMural = sobreMural.filter((_, i) => i !== index);
    setSobreMural(novoMural);
    localStorage.setItem("sobre-mural", JSON.stringify(novoMural));
  };
  ...
  return (
    <div style={{ padding: "20px", maxWidth: "650px", margin: "0 auto" }}>
      ...
      {carregandoMidia && (
        <div style={{ position: "fixed", top: 20, right: 20, background: "#000", color: "#fff", padding: "12px 24px", borderRadius: "8px", zIndex: 9999, fontWeight: "bold" }}>
          ⏳ Subindo arquivo...
        </div>
      )}
      ...
      <div style={boxStyle}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "15px" }}>
          ...
          {editandoId && <button onClick={limparFormProduto} style={botaoCancelar}>CANCELAR EDIÇÃO ×</button>}
        </div>

        <input placeholder="Nome *" value={nome} onChange={e => setNome(e.target.value)} style={inputStyle} />
        <input placeholder="Preço (ex: 35,90)" value={preco} onChange={e => setPreco(e.target.value)} style={inputStyle} />
        <textarea placeholder="Descrição" value={desc} onChange={e => setDesc(e.target.value)} style={{ ...inputStyle, minHeight: "80px" }} />

        <div style={subSecaoStyle}>
          <p style={labelSecao}>🔗 Opção A — URL externa</p>
          <input placeholder="Link da imagem principal" value={img} onChange={e => setImg(e.target.value)} style={inputStyle} />
          {imagens.map((url, i) => (
            <div key={i} style={{ display: "flex", gap: "8px", marginBottom: "8px" }}>
              <input placeholder={`Link adicional ${i + 1}`} value={url} onChange={e => atualizarImagem(i, e.target.value)} style={{ ...inputStyle, marginBottom: 0, flex: 1 }} />
              <button onClick={() => removerImagemLink(i)} style={botaoRemover}>×</button>
            </div>
          ))}
          <button onClick={adicionarCampoImagem} style={botaoDashed}>+ Adicionar campo de link</button>
        </div>

        <div style={subSecaoStyle}>
          <p style={labelSecao}>📁 Opção B — Upload do dispositivo</p>
          <label style={fileLabelStyle}>
            📷 Imagem principal
            <input type="file" accept="image/*,video/*" onChange={async e => {
              if (e.target.files?.[0]) { const url = await fazerUploadArquivo(e.target.files[0]); if (url) setArquivoLocalImg(url); }
            }} style={{ display: "none" }} />
          </label>
          {arquivoLocalImg && (
            <div style={previewContainer}>
              {ehVideo(arquivoLocalImg)
                ? <video src={arquivoLocalImg} style={previewMedia} controls muted />
                : <img src={arquivoLocalImg} style={previewMedia} alt="" />}
              <button onClick={() => setArquivoLocalImg("")} style={botaoRemoverPreview}>✕</button>
            </div>
          )}

          <label style={{ ...fileLabelStyle, marginTop: "10px" }}>
            🗂️ Imagens adicionais (múltiplas)
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(90px, 1fr))", gap: "10px", marginTop: "12px" }}>
              {arquivosLocaisAdicionais.map((url, i) => (
                <div key={i} draggable
                  ...
                  onDrop={e => { const origem = Number(e.dataTransfer.getData("index")); moverArquivo(origem, i); }}
                  style={{ ...previewContainer, cursor: "grab" }}
                >
                  {ehVideo(url)
                    ? <video src={url} style={previewMedia} muted playsInline />
                    : <img src={url} style={previewMedia} alt="" />}
                  <button onClick={() => removerArquivoAdicional(i)} style={botaoRemoverPreview}>✕</button>
                </div>
              ))}
            </div>
          )}
        ...
        <button onClick={salvarProduto} style={botaoVerde}>
        ...
        {produtos.map(p => (
          <div key={p.id} style={cardStyle}>
            <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
              {p.img && <img src={p.img} style={{ width: "45px", height: "45px", objectFit: "contain", borderRadius: "8px" }} alt="" />}
              <div>
      ...
      <div style={boxStyle}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "15px" }}>
          ...
          {editandoCategoria && <button onClick={limparFormCategoria} style={botaoCancelar}>CANCELAR ×</button>}
        </div>

        <input placeholder="Nome da categoria *" value={nomeCategoria} onChange={e => setNomeCategoria(e.target.value)} style={inputStyle} />

        <div style={subSecaoStyle}>
          <p style={labelSecao}>🔗 URL da imagem (usada na busca)</p>
          <input placeholder="Cole o link da imagem" value={imagemCategoria} onChange={e => setImagemCategoria(e.target.value)} style={inputStyle} />
        </div>

        <div style={subSecaoStyle}>
          <p style={labelSecao}>📁 Upload do dispositivo</p>
          <label style={fileLabelStyle}>
            📁 Escolher imagem
            <input type="file" accept="image/*" onChange={async e => {
              if (e.target.files?.[0]) { const url = await fazerUploadArquivo(e.target.files[0]); if (url) setArquivoLocalCategoria(url); }
            }} style={{ display: "none" }} />
          </label>
          {arquivoLocalCategoria && (
            <div style={previewContainer}>
              <img src={arquivoLocalCategoria} style={previewMedia} alt="" />
              <button onClick={() => setArquivoLocalCategoria("")} style={botaoRemoverPreview}>✕</button>
            </div>
          )}
        </div>

        <button onClick={salvarCategoria} style={botaoAzul}>
      ...
      <div style={boxStyle}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "15px" }}>
          ...
          {editandoColecao && <button onClick={limparFormColecao} style={botaoCancelar}>CANCELAR ×</button>}
        </div>

        <input placeholder="Nome da coleção *" value={nomeColecao} onChange={e => setNomeColecao(e.target.value)} style={inputStyle} />

        <div style={subSecaoStyle}>
          <p style={labelSecao}>🔗 URL do ícone</p>
          <input placeholder="Cole o link do ícone" value={iconeColecao} onChange={e => setIconeColecao(e.target.value)} style={inputStyle} />
        </div>
        ...
        <div style={subSecaoStyle}>
          ...
          <label style={fileLabelStyle}>
            📁 Escolher ícone
            <input type="file" accept="image/*" onChange={async e => {
              if (e.target.files?.[0]) { const url = await fazerUploadArquivo(e.target.files[0]); if (url) setArquivoLocalColecao(url); }
            }} style={{ display: "none" }} />
          </label>
          {arquivoLocalColecao && (
            <div style={previewContainer}>
              <img src={arquivoLocalColecao} style={previewMedia} alt="" />
              <button onClick={() => setArquivoLocalColecao("")} style={botaoRemoverPreview}>✕</button>
            </div>
          )}
        </div>

        <label style={{ fontSize: "13px", color: "#555", marginBottom: "6px", display: "block" }}>Cor do card</label>
        ...
        <button onClick={salvarColecao} style={botaoLaranja}>
          {editandoColecao ? "✅ Salvar alterações" : "➕ Adicionar coleção"}
        </button>
        ...
        {colecoes.map(c => (
          <div key={c.id} style={cardStyle}>
            <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
              ...
              <div>
                <strong>{c.nome}</strong>
                {c.fixar_home && <p style={{ margin: 0, fontSize: "11px", color: "green" }}>✔ Fixada na home</p>}
              </div>
            </div>
            <div style={{ display: "flex", gap: "5px" }}>
      ...
      <div style={boxStyle}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "15px" }}>
          ...
          {editandoBanner && <button onClick={limparFormBanner} style={botaoCancelar}>CANCELAR ×</button>}
        </div>

        <input placeholder="Buscar produto..." value={buscaBanner} onChange={e => setBuscaBanner(e.target.value)} style={inputStyle} />
        {sugestoesBanner.length > 0 && (
          <div style={{ border: "1px solid #ddd", borderRadius: "10px", background: "#fff", marginBottom: "15px", maxHeight: "150px", overflowY: "auto" }}>
        ...
        {produtoSelecionado && (
          <div style={{ background: "#eef7ff", padding: "10px", borderRadius: "10px", marginBottom: "15px", display: "flex", justifyContent: "space-between" }}>
            <span>Selecionado: <strong>{produtoSelecionado.nome}</strong></span>
            <span onClick={() => setProdutoSelecionado(null)} style={{ cursor: "pointer" }}>✕</span>
          </div>
        )}

        <label style={{ fontSize: "13px", color: "#555", marginBottom: "6px", display: "block" }}>Cor do banner</label>
        ...
        <button onClick={salvarBanner} style={botaoRoxo}>
          {editandoBanner ? "✅ Salvar alterações" : "➕ Adicionar banner"}
        </button>
          ...
          <div key={b.id} style={cardStyle}>
            <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
              ...
              <div>
                ...
                <div style={{ width: "30px", height: "12px", background: b.cor, borderRadius: "4px", marginTop: "4px" }} />
              </div>
            </div>
            <div style={{ display: "flex", gap: "5px" }}>
      ...
      <div style={boxStyle}>
        ...
        <textarea
          placeholder="Conte a história da loja, sua origem, valores..."
          value={sobreTexto}
          onChange={e => setSobreTexto(e.target.value)}
          style={{ ...inputStyle, minHeight: "140px" }}
        />

        <button onClick={salvarSobreTexto} style={botaoVerde}>
          💾 Salvar texto
        </button>

        <h3 style={{ marginTop: "30px" }}>Mural de Criações</h3>
        <p style={{ fontSize: "13px", color: "#666", marginBottom: "10px" }}>
          Fotos que aparecem no mural da página Sobre.
        </p>

        <div style={subSecaoStyle}>
          <p style={labelSecao}>🔗 URL externa</p>
          <input
            placeholder="Cole o link da imagem"
            value={novaMuralUrl}
            onChange={e => setNovaMuralUrl(e.target.value)}
            style={inputStyle}
          />
        </div>
        ...
        <div style={subSecaoStyle}>
          ...
          <label style={fileLabelStyle}>
            📷 Escolher imagem
            <input type="file" accept="image/*" onChange={async e => {
              if (e.target.files?.[0]) { const url = await fazerUploadArquivo(e.target.files[0]); if (url) setArquivoMural(url); }
            }} style={{ display: "none" }} />
          </label>
          {arquivoMural && (
            <div style={previewContainer}>
              <img src={arquivoMural} style={previewMedia} alt="" />
              <button onClick={() => setArquivoMural("")} style={botaoRemoverPreview}>✕</button>
            </div>
          )}
        </div>

        <button onClick={adicionarFotoMural} style={botaoAzul}>
          ➕ Adicionar foto ao mural
        </button>

        {sobreMural.length > 0 && (
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "10px", marginTop: "20px" }}>
            {sobreMural.map((url, i) => (
              <div key={i} style={{ position: "relative" }}>
                <img src={url} alt="" style={{ width: "100%", aspectRatio: "1", objectFit: "cover", borderRadius: "12px" }} />
                <button onClick={() => removerFotoMural(i)} style={{ ...botaoRemoverPreview, top: "5px", right: "5px", padding: "3px 8px", fontSize: "12px" }}>✕</button>
              </div>
            ))}
          </div>
        )}
...
const subSecaoStyle = { background: "#fdfdfd", border: "1px solid #eaeaea", padding: "15px", borderRadius: "12px", marginBottom: "15px" };
...
const labelSecao = { fontWeight: "bold", margin: "0 0 8px 0", fontSize: "13px", color: "#2c3e50" };
...
const previewContainer = { position: "relative", marginTop: "10px", width: "90px", height: "90px", border: "1px solid #ddd", borderRadius: "8px", overflow: "hidden", background: "#f7f7f7", display: "flex", alignItems: "center", justifyContent: "center" };
const previewMedia = { width: "100%", height: "100%", objectFit: "contain" };
const botaoRemoverPreview = { position: "absolute", top: "2px", right: "2px", background: "rgba(230,57,70,0.9)", color: "white", border: "none", borderRadius: "4px", padding: "2px 6px", fontSize: "10px", cursor: "pointer" };
const botaoRemover = { backgroundColor: "#E63946", color: "white", border: "none", borderRadius: "8px", padding: "0 14px", cursor: "pointer" };
const botaoDashed = { width: "100%", padding: "10px", backgroundColor: "#f9f9f9", border: "1px dashed #bbb", borderRadius: "10px", cursor: "pointer", fontSize: "13px" };
...
const botaoCancelar = { backgroundColor: "#E63946", color: "white", border: "none", borderRadius: "8px", padding: "6px 12px", cursor: "pointer", fontSize: "12px", fontWeight: "bold" };