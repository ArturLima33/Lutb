          ...
          {colecoesOrdenadas.map((c) => (
            <Link href={`/colecao/${c.id}`} key={c.id} style={{ textDecoration: "none" }}>
              <div style={{ backgroundColor: "white", borderRadius: "25px", padding: "20px 25px", boxShadow: "0 4px 10px rgba(0,0,0,0.1)", display: "flex", alignItems: "center", gap: "20px", minHeight: "60px" }}>
                {c.icone && <img src={c.icone} style={{ width: "60px", height: "60px", objectFit: "contain", borderRadius: "8px" }} alt="" />}
                <div>
                  <h3 style={{ fontSize: "22px", margin: 0, fontFamily: "serif", color: "#2D2D1A", fontWeight: "normal" }}>{c.nome}</h3>
                  {c.fixar_home && <p style={{ margin: "4px 0 0 0", fontSize: "12px", color: "green" }}>✔ Fixada na home</p>}
                </div>
              </div>
            </Link>
          ))}
